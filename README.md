...
默认在 `http://localhost:3000` 打开。首次访问需输入首页密码。

4. 重要说明

- 开发模式下 API 运行在 Edge Runtime 模拟环境，无需 Node.js 特权模块支持。
- 数据存储模式默认为 `localstorage`。如需开启云存储（Upstash/D1/Redis），请设置对应环境变量并参考下文。
...
构建过程中会执行 `pnpm gen:runtime`，将 `config.json` 转换为 `src/lib/runtime.ts`，用于在 Edge 环境读取运行时配置。
...
请在 EdgeOne Pages 控制台创建项目并配置以下内容：
...
- 输出目录：`out`（已配置 `output: 'export'`，构建后生成静态目录）。

- 路由/缓存（示例）：
  - HTML：no-cache
  - 静态资源：cache-control: public, max-age=31536000, immutable
  - API：短缓存（例如 s-maxage=60）

EdgeOne Pages 仅托管静态资源；若需要接口能力，请改用支持 Functions 的平台。豆瓣数据建议配置前端代理前缀以避免 CORS：在 UI 设置中开启“豆瓣代理”，或在 `config.json` 增加 `douban_proxy` 字段，形如：
...
所有可自定义项集中在根目录的 `config.json` 中（新增首页明文密码 `homepage_password`）：
...
修改后 **无需重新构建**，服务会在启动时读取一次。`homepage_password` 将在运行时注入到 `window.RUNTIME_CONFIG.HOMEPAGE_PASSWORD`，并用于登录与中间件密码校验（优先级：`config.json` > 环境变量 `PASSWORD`）。
